...
void main(){
    ...
    float heightMask = clamp(csm_Position.y + 0.5, 0.0, 1.0);
    heightMask = pow(heightMask, 1.5);

    float smallNoise = fbm(
        (worldPos + csm_Position) * uSmallWindScale + vec3(uTime * uSmallWindSpeed)
    ) - 0.5;

    float largeNoise = fbm(
        (worldPos + csm_Position) * uLargeWindScale + vec3(uTime * uLargeWindSpeed)
    ) - 0.5;
    ...
    vec3 billboardVertexPosition = worldPos
        + (cameraRight * csm_Position.x * scaleX)
